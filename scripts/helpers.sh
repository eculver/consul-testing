...
function vm_install_consul {
  ...
  #
  # I did end up making the vm name resolution thing, see replace-consul-bin.sh
  # but still want something that works for cloud VMs too
  # 
  # months later... with a fresh pair of eyes and his wits about him (see install_consul below)
}

# Installs a specific version of consul to the given host by downloading a binary to $CONSUL_BINS_HOME (if necessary)
# before copying to the host using scp. We might later do some vm name resolution here, but
# typically the operator (me) knows the host IP so just require it and do it the old fashioned way with SCP.
function install_consul {
  ...
  echo "Installing Consul ${version} to ${ip}:${path}"
